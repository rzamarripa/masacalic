	...
	if ($model->estatus_did == 1 || $model->estatus_did == 2){
		$this->menu=array(
			...
			array('label'=>'Crear Planeación', 'url'=>array('create', 
							...
							'tipoUsuario' => 2)),
			array('label'=>'Actualizar Planeación', 'url'=>array('update', 'id'=>$model->id, 'tipoUsuario'=> $tipoUsuario[0]["id"], 'matid' => $model->materia->id, 'maestroid' => $model->maestro->id,)),
		);
	}
	...
	if ($model->estatus_did == 1 || $model->estatus_did == 2){
		$this->menu=array(
			array('label'=>'Volver a mis planeaciones', 'url'=>array('planporcoord', 
							...
							'maestroid' => $model->maestro->id,
							'usuario' => $model->maestro->usuario->usuario)),
			array('label'=>'Actualizar Planeación', 'url'=>array('update', 'id'=>$model->id, 'tipoUsuario'=> $tipoUsuario[0]["id"], 'materia' => $model->materia->nombre, 
							'matid' => $model->materia->id,'maestroid' => $model->maestro->id, )),
		);
	}
	elseif($model->estatus_did == 3 || $model->estatus_did == 4){
		$this->menu=array(
...
<?php $this->widget('zii.widgets.CDetailView', array(
	...
	'attributes'=>array(
        ...
        array(               // related city displayed as a link
            'label'=>'Competencias Procedimientales',
            'type'=>'raw',
        ...
        array(               // related city displayed as a link
            'label'=>'Metacognición',
            'type'=>'raw',
            'value'=>'<pre>' . $model->metacognicion . '</pre>',
        ),
        array(               // related city displayed as a link
            'label'=>'Estatus',
            'type'=>'raw',
