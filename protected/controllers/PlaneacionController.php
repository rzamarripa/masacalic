...
class PlaneacionController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('index','view','create','update','admin','delete','misplaneaciones','planporcoord','planporadmin','cambiarestatus','detalle','reiniciar'),
				'users'=>array('@'),
			),
			/*
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Planeacion']))
		{
			...
			$relacion = MateriaMaestro::model()->find('materia_aid = :mat && maestro_aid = :maes', array(':mat' => $model->materia_aid, 'maes'=> $model->maestro_aid));
			$model->materiaMaestro_did = $relacion->id;
			if($model->save())
			{				
				...
				{
					$body = 'El maestro ' . $model->maestro->nombre . ' ha realizado una nueva planeación en la materia de ' . 
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Planeacion']))
		{
			...
			if($model->save())
			{
				//Aquí se envía cuando liberan una planeación
				$usuarioActual = Usuario::model()->find('usuario = :x', array(':x' => Yii::app()->user->name));
				if($usuarioActual->tipoUsuario->id == 2 && $model->estatus->id == 1)
				{
